...
class MetadataDiscovery {
    ...
    async discover(preferTsNode = true) {
        ...
        filtered.forEach(meta => this.metadata.set(meta.className, metadata_1.EntitySchema.fromMetadata(meta).init().meta));
        filtered.forEach(meta => this.defineBaseEntityProperties(meta));
    ...
    async discoverDirectories(paths) {
        ...
        for (const filepath of files) {
            ...
            if (!(target instanceof Function) && !(target instanceof metadata_1.EntitySchema)) {
                this.logger.log('discovery', `- ignoring file ${filename}`);
                continue;
            }
    ...
    prepare(entity) {
        if ('schema' in entity && entity.schema instanceof metadata_1.EntitySchema) {
    ...
    initEmbeddables(meta, embeddedProp) {
        if (embeddedProp.reference !== entity_1.ReferenceType.EMBEDDED) {
            return;
        }
    ...
    initSingleTableInheritance(meta) {
        ...
        if (!root.discriminatorMap) {
            root.discriminatorMap = {};
            const children = Object.values(this.metadata.getAll()).filter(m => utils_1.Utils.getRootEntity(this.metadata, m) === root);
            children.forEach(m => {
                const name = m.discriminatorValue || this.namingStrategy.classToTableName(m.className);
                root.discriminatorMap[name] = m.className;
            });
        }
        meta.discriminatorValue = (_a = Object.entries(root.discriminatorMap).find(([, className]) => className === meta.className)) === null || _a === void 0 ? void 0 : _a[0];
        ...
        Object.values(meta.properties).forEach(prop => {
            ...
            root.properties[prop.name] = utils_1.Utils.copy(prop);
            root.properties[prop.name].nullable = true;
            if (!exists) {
                root.properties[prop.name].inherited = true;
            }
        });
        root.indexes = utils_1.Utils.unique([...root.indexes, ...meta.indexes]);
        root.uniques = utils_1.Utils.unique([...root.uniques, ...meta.uniques]);
    }
    ...
    initEnumValues(prop, path) {
        ...
        const exports = utils_1.Utils.requireFrom(path, process.cwd());
    ...
    getEntityClassOrSchema(path, name) {
        ...
        const schema = Object.values(exports).find(item => item instanceof metadata_1.EntitySchema);
        if (schema) {
            return schema;
        }
        if (!target) {
